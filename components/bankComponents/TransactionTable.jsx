...
const generateTransactions = () => {
  const descriptions = [
    "Truworths", "Mr Delivery", "Intrepid Travel", "Semicolon Bookstore",
    "Woolworths", "Donnies Locksmith", "Bmw Financial Services", 
    "Tiger Wheel and Tyre", "Cool ideas", "Engen", "Outsurance",
    "Pet City", "Fly Safair", "Liberty Life", "Doppio Zero"
  ];
  ...
  return Array.from({ length: 100 }, (_, i) => ({
    ...
    reference: "11286465959",
    serviceFee: "R 0.00",
    amount: "R 2 000.00",
    availableBalance: "R 2 000.00"
  }));
};
...
const TransactionTable = () => {
  ...
  const data = useMemo(() => generateTransactions(), []);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: "Service Fee",
        cell: ({ row }) => <div>{row.getValue("serviceFee")}</div>,
      },
      {
        accessorKey: "amount",
        header: "Amount",
      ...
      },
      {
        ...
        cell: ({ row }) => <div>{row.getValue("availableBalance")}</div>,
      },
    ],
    []
  );
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      {/* Filters */}
        {filterOpen && (
          <TransactionFilters toggleFilter={setFilterOpen} />
        )}
